...
- `@internal` (first word) marks symbols that are not part of the public API;
  `tsconfig.build.json` sets `stripInternal: true`, so they are erased from the emitted
  `.d.ts`. A class constructed through a static factory uses a `private constructor` rather
  than `@internal` (see `Injector.create()`) — `private` keeps the constructor in the `.d.ts`
  to block external `new`, whereas `@internal` would delete it and let `new Injector()` compile.
